...
aws ec2 create-image --instance-id $(cat instance.id) --name $IMAGE_NAME | jq -r .ImageId > image.id
...
aws ec2 wait image-available --filters Name=name,Values=$IMAGE_NAME
